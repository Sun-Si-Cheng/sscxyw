...
$perPage = 20;
$list = getFollowList($userId, 'following', $page, $perPage);
$total = getFollowListCount($userId, 'following');
$totalPages = ceil($total / $perPage);

$pageTitle = clean($profileUser['nickname'] ?: $profileUser['username']) . ' 的关注';
include __DIR__ . '/includes/header.php';
?>

<div class="container" style="max-width: 600px; margin: 2rem auto;">
    <div class="profile-form-card">
        <h3><i class="fas fa-user-friends"></i> 关注列表</h3>
        <p class="text-muted"><a href="user_profile.php?id=<?php echo $userId; ?>"><?php echo clean($profileUser['nickname'] ?: $profileUser['username']); ?></a> 的关注 (<?php echo $total; ?>)</p>
        <?php if (empty($list)): ?>
        <p class="text-muted">暂无关注</p>
        <?php else: ?>
